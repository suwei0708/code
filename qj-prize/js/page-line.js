$(function() {
    ...
    $("#prize-btn").on('click', function(){
        ...
        if(luck.num > 0) {
            ...
            $('#luck-num').html(luck.num);
            luck.speed = 100;
            $.get("/Zhounian/Cjvip",function(data){
                if(data==110){//未登录
                    loginshow();
                }
                else if(data==10){//抽奖次数不够
                    //loginshow();
    ...
    $('.btn-give').on('click', function() {
        ...
        $.get("/Zhounian/Sendvip",function(data){
            if(data==0){
                $('.popup-give').find('.tit').html('赠送失败(┬＿┬) 您已经赠送过一次了哦');
                 ...
                 $('.popup-give').find('.tit').html('自己不能给自己赠送');  // 赠送结果文字
            }
            else{
                 $('.popup-give').find('.tit').html('您尚未登陆,不能参与赠送');  // 赠送结果文字
            }
        })
        ...
        timer = setInterval(function() {
            ...
            if(time == 0) {
                ...
                $('.popup-give').hide();
                mask.hide();
                location.href='http://www.qiaojiang.tv';
            }
        }, 1000);
    });
...
function center(obj) {
    ...
    if(cssname=="popup popup-give"){
    $(obj).css({
        'margin-top': -($('.popup-pos').height() )
    });
    console.log(-($('.popup-pos').height() - $(obj).height()) / 2)
    }else{
    $(obj).css({
